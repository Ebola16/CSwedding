...
function finishLoading() {
  ...
  if (overlay) {
    ...
    }, 200); // slight delay for smooth sequencing
  }
...
function spawnTravelingGif(gifs) {
  if (document.hidden || !allLayersReady) {
    setTimeout(() => spawnTravelingGif(gifs), 2000 + Math.random() * 4000);
    return;
  }
  const gif = getGif();
  const useShiny = Math.random() < (1 / 4096) ? 'shiny' : 'normal';
  const randomGif = "0370 - Luvdisc.gif";
